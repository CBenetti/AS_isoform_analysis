...
###function
	...
	##What edgeR suggests
		x <- exprs(annotated_genes_counts_cell_bank)[,cells]
		...
		if(is.data.frame(DGE_res)==TRUE){
			genes <- rownames(DGE_res)[which(DGE_res$FDR < 0.05 & abs(DGE_res$logFC)>2)]
...
top_annotation=col_anno,left_annotation=row_anno,column_title=title,row_title=NULL,show_row_names=FALSE,show_column_names=FALSE,row_split=row_split,column_split=columns_split))
			...
			genes <- lapply(DGE_res,function(x){rownames(x$table)[which(x$table$FDR < 0.05 & x$table$logFC>2)]})
...
##MS status
	...
	DGE_res <- DGE$MS_status_adjusted$table
...
## cetuximab DGE
	...
	DGE_res <- DGE$Cetuximab_adjusted$table
...
## CIMP
	...
	DGE_res <- DGE$CIMP_adjusted$table
