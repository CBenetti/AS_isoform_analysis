...
cpm <- 
apply(exprs(annotated_genes_counts_cell_bank),2,function(x){x*1000000/sum(x)})
HNRNPK <- 
cpm[which(fData(annotated_genes_counts_cell_bank)$Geneid=="HNRNPK"),colnames(sub)]
l2mHNRNPK <- scale(HNRNPK)	
...
cetuximab=anno_simple(pData(isoforms_cell_bank$isoform_proportions)$cetuximab_sensitivity[grep("onlyCRIS",pData(isoforms_cell_bank$isoform_proportions)$CRIS_class)],col=co1),
MSI_MSS=anno_simple(pData(isoforms_cell_bank$isoform_proportions)$MS_status[grep("onlyCRIS",pData(isoforms_cell_bank$isoform_proportions)$CRIS_class)],col=co3),HNRNPK=l2mHNRNPK),
